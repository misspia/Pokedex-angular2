...
fs.unlink(filepath, cb => {
	...
	for (let i = 0; i < abilitiesDict.length; i++) {
		...
		let formattedString = 'INSERT into pokedex.abilities_description VALUES (\'' + abilitiesDict[i].Name + '\', ' + abilitiesDict[i].Pokemon + ', \'' + abilitiesDict[i].Description + '\', ' + abilitiesDict[i].Generation + ');\n';
	...
	// 		movesDict[i].Accuracy + '\", \"'  + movesDict[i].PP + '\", \"' + movesDict[i].TM + '\", \"' + insert.normalizeString(movesDict[i].Effect) + '\", \"' + movesDict[i].Probability + '\");\n';
